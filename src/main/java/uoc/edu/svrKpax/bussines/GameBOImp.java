...
public class GameBOImp implements GameBO {
	...
	@Override
	public boolean modifyGame(String campusSession, String name, int idGame,
			int idCategory, int publicAcces) {
		try {
			
			if (sBo.validateSession(campusSession) != null)
					
			{
				Game objGameold = null;
				objGameold = gDao.getGame(idGame);	
				//idGame = objGameold.getIdGame();
				CreationDate = objGameold.getCreationDate();
				SecretGame = objGameold.getSecretGame();
				PrivateKey = objGameold.getPrivateKey();
							
				Game objGame = new Game();
				objGame.setIdGame(idGame);
				objGame.setName(name);
				objGame.setPublicAcces(publicAcces);
				objGame.setSecretGame(SecretGame);
				objGame.setPrivateKey(PrivateKey);
				objGame.setIdCategory(idCategory);
				objGame.setCreationDate(CreationDate);
